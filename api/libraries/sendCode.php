...
try {
    // Obtener el nombre y apellido del cliente basado en el correo
    $query = "SELECT nombre_cliente, apellido_cliente FROM clientes WHERE correo_cliente = ?";
    $result = Database::getRow($query, [$email]);

    if ($result) {
        $nombre = $result['nombre_cliente'];
        $apellido = $result['apellido_cliente'];
    } else {
        echo json_encode(['status' => false, 'message' => 'Correo no encontrado']);
        exit();
    }
    ...
    $html = "<html>
        ...
        <body>
            <div class='container'>
                ...
                <div class='footer'>Soccer Live</div>
            </div>
        </body>
        </html>";
    $mail->Body = $html;
